...
public class JavaGenerator : CodeGeneratorBase
{
    ...
    public override string AppendToExistingFile(string existingContent, CsvFileData newData, string className)
    {
        ...
        // Son "));", ");" veya "))" satırını bul (liste kapanışı)
        int insertIndex = -1;
    ...
    private void GenerateLookupData(StringBuilder sb, CsvFileData data, string className, int lookupKeyColumnIndex)
    {
        ...
        var keyBoxed = keyColumn.CSharpType == "enum" ? GetEnumTypeName(keyColumn) : MapBoxedType(keyColumn.CSharpType);
        var groups = BuildGroups(data, lookupKeyColumnIndex);
        var propName = SanitizeIdentifier(keyColumn.PropertyName);

        sb.AppendLine($"    public static final Map<{keyBoxed}, List<{className}>> itemsBy{propName} = new HashMap<>() {{{{");
        for (int gi = 0; gi < groups.Count; gi++)
        {
            var group = groups[gi];
            ...
            sb.AppendLine($"        put({keyLiteral}, new ArrayList<>(Arrays.asList(");
            for (int ri = 0; ri < group.Rows.Count; ri++)
                GenerateObjectCreation(sb, data, group.Rows[ri], className, "            ", ri < group.Rows.Count - 1);
            sb.AppendLine("        )));");
        }
    ...
    private void GenerateUniqueArrays(StringBuilder sb, CsvFileData data)
    {
        ...
        foreach (var column in uniqueColumns)
        {
            ...
            var typeName = column.CSharpType == "enum" ? GetEnumTypeName(column) : MapBoxedType(column.CSharpType);
            var uniqueValues = GetUniqueColumnValues(data, column.ColumnIndex);
            ...
            sb.AppendLine($"    public static final List<{typeName}> unique{propName} = new ArrayList<>(Arrays.asList({string.Join(", ", formatted)}));");
        }
        sb.AppendLine();
    }
